...
class Engine {
    constructor ({ store = localStore()}) {
        ...
        this.use(this._engineMiddleware)
    }
    ...
    async _engineMiddleware ({ action, state }, next) {
    ...
    async _act (stateAction, action, context) {
      ...
      await plugin.actions[actionType](this._processJsonPath(stateAction.spec, context), action, context)
    }

    async _dispatchToFlow(flow, action, context = {}) {
      const actionAcceptor = flow
        .startAcceptors(action)
        .find(acceptor => action.acceptor(this._processJsonPath(acceptor.spec, context)));
      ...
      const { middleware: actionMiddleware, next, parentState: acceptorState } = actionAcceptor;
      ...
      await Promise.all(nextState.actions.map(stateAction => this._act(stateAction, action, context)));
    }

    async dispatch (action) {
        action.acceptor = this._getActionAcceptor(action).bind(null, action);
